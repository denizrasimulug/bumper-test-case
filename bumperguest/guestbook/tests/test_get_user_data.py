...
class GetUserDataTestCase(TestCase):
    ...
    def test_get_user_data_latest_entry(self):
        ...
        users = response.json()['users']
        self.assertEqual(users[0]['last_entry'], f"{latest_entry.subject} | {latest_entry.message}")
    ...
    def test_get_user_data_no_entries(self):
        ...
        users = response.json()['users']
        self.assertEqual(len(users), 0)  # Check that no users are returned if no entries exist
