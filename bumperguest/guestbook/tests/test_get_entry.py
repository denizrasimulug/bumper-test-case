...
class GetEntriesTestCase(TestCase):
    ...
    def test_get_entries_invalid_page(self):
        response = self.client.get(
            reverse("entry-view") + "?page=100"
        )  # Invalid page number
        self.assertEqual(response.status_code, 400)
        self.assertIn(
            "errors", response.json()
        )  # Assuming your request_data.is_valid() will fail for invalid page

    def test_get_entries_pagination(self):
        response = self.client.get(
            reverse("entry-view") + "?page=2"
        )  # Get the second page
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            len(response.json()["entries"]), 2
